--[[
    MM2 - iOS 26 Liquid Glass (Усиленный эффект капли)
    Круглая иконка меню (ID 6159452397) с анимацией
    - Реалистичная анимация: из кнопки вылетает капля (овал), затем раскрывается в прямоугольник
    - Кнопка меню увеличивается при нажатии
    - "Стать убийцей" и "Стать шерифом" — переключатели (авто-подбор оружия)
    - Aimbot для шерифа (авто-прицеливание)
    - "Стрельба сквозь стену" — переключатель, авто-стрельба по убийце
    - Скорость и прыжок не сбрасываются при подборе оружия
    - Кнопки качаются при скроллинге
    - Сохранение и загрузка всех настроек в файл mm2_settings.txt
    - Телепорт к оружию (пистолет/ружьё)
    - Все надписи на русском
    - Меры против кика 267
--]]
...
local function openPanel()
    ...
    -- Этап 1: из кнопки вылетает капля (овал) в центр экрана
    local dropletSize = UDim2.new(0, 80, 0, 260)  -- высокая капсула
    ...
    local tween1 = TweenService:Create(mainPanel, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        Size = dropletSize,
        Position = centerPos,
        BackgroundTransparency = 0.4  -- немного проявимся
    })
    tween1:Play()
    
    -- Этап 2: капля расширяется в полноценное меню
...
setMini()
mainPanel.Visible = false
print("✅ Эффект жидкой капли усилен! Меню вылетает овалом и раскрывается.")
